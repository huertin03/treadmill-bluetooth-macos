...
- **Delegate:** research (`~/.claude/tools/codex-agent.sh --ro <name> "<question>"`) and self-contained, well-scoped implementation (`~/.claude/tools/codex-agent.sh <name> "<task>"`) — run from inside this repo. The wrapper pins GPT-6 Astra @ effort `low` and prefixes the brief with "this brief IS the task doc — implement, test, commit now"; do not fight either. Criterion: **expected effort, not diff size** — delegate when the work needs exploration not yet done (reading more than 2–3 files to know where the change goes), a test-fix loop (compile/test/repeat), or new code plus tests of ≥ ~200 lines; do it yourself when the whole change can be written in one pass from what is already in context (up to ~150 lines) and the operator is waiting. While the weekly GPT-6 Astra usage sits below Fable's, resolve doubt towards delegating.
...
- **Decide before the first `Edit`, and say so:** the criterion is judged up front, never after the work is done; if work that meets the criterion stays in the main thread, state the reason in one line (your `/effort` level is never the reason). In-session `Agent` subagents are for reading, search, review and web research — work that meets the criterion goes through `codex-agent.sh`, not through a subagent. Brief hygiene: formatting (`cargo fmt`, prettier) as a separate commit; one run ≤ ~20 min; on exit 2 inspect the worktree (`git -C ../<repo>_codex-<name> status`) before choosing `--resume` — finishing by hand is the exception, not the lesson.
- **Remote execution never goes to the executor:** SSH, `kubectl`, mesh/VPN, `adb`, secret stores, provider CLIs — anything that reaches another machine or a live service — runs in the Claude Code session under its command classifier; the executor runs with approvals bypassed inside `srt` and only writes files and runs local gates.
...
- **Run independent phases in parallel** — there is no concurrency cap: every wrapper serialises worktree add/remove on one shared lock and pins its worktree for the run. The limits are the machine and the weekly Codex quota — **no per-run cap** (ADR 007 in `ankor-dotfiles`); the preflight refuses to start below 5 pp remaining, and then the same brief runs on `grok-agent.sh`.
...
- **Trust the diff, not the report:** verify the actual commits on `codex/<name>` and re-run the gates yourself. Every run leaves `.git/codex-runs/<name>-<utc>.{report.md,metrics.json}` (verified model/effort, tokens, commands, quota delta). Exit 2 (aborted / planning-only) → continue with `--resume <name>` rather than restarting; exit 1 on a model or effort mismatch is deliberate — never work around it.
