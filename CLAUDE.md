...
- **Decide before the first `Edit`, and say so:** the threshold is judged up front, never after the work is done; if threshold-crossing work stays in the main thread, state the reason in one line (your `/effort` level is never the reason). In-session `Agent` subagents are for reading, search, review and web research — repository writes ≥ the threshold go through `codex-agent.sh`, not through a subagent. Brief hygiene: formatting (`cargo fmt`, prettier) as a separate commit; one run ≤ ~20 min; on exit 2 inspect the worktree (`git -C ../<repo>_codex-<name> status`) before choosing `--resume` — finishing by hand is the exception, not the lesson.
