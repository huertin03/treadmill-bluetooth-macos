...
//! threshold. This module owns two orthogonal, individually-testable pieces:
//! mapping thresholds to celebration tiers, and the pure crossing decision.
//! Loading the per-user TOML config lives in [`crate::config`]; persisting
//! *which* goals were already celebrated today (restart safety) lives in
//! [`crate::store`]; delivering the toast lives in [`crate::notify`].
//!
//! A missing file is normal and falls back to the compiled defaults.

use std::collections::HashSet;

use tracing::{info, warn};

use crate::config::file::config_path;
