...
pub struct ZoomConfig { pub enabled: bool, pub delta_pt: f64 }
impl Default for ZoomConfig {
    fn default() -> Self { Self { enabled: false, delta_pt: DEFAULT_ZOOM_PT } }
}
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct ZoomWant { pub config: ZoomConfig, pub active: bool }
#[derive(Debug, Clone, Copy, PartialEq)]
#[allow(dead_code)]
pub enum Applied { Unknown, Base, Zoomed { delta_pt: f64 } }
#[derive(Debug, Clone, Copy, PartialEq)]
pub enum ZoomOp { Revert, Apply { delta_pt: f64 } }

#[allow(dead_code)]
pub fn plan(applied: Applied, want: &ZoomWant) -> Option<ZoomOp> {
    if want.config.enabled && want.active {
        if applied == (Applied::Zoomed { delta_pt: want.config.delta_pt }) { return None; }
        return Some(ZoomOp::Apply { delta_pt: want.config.delta_pt });
    }
    if applied == Applied::Base { return None; }
    Some(ZoomOp::Revert)
}
...
pub fn is_valid_delta(pt: f64) -> bool { pt.is_finite() && pt > 0.0 && pt <= MAX_ZOOM_PT }
pub fn sizes_match(a: f64, b: f64) -> bool { (a - b).abs() <= SIZE_EPSILON }
...
pub fn parse_font_size(json: &str) -> Option<f64> {
    serde_json::from_str::<serde_json::Value>(json).ok()?
        .get("font")?.get("size")?.as_f64().filter(|pt| pt.is_finite() && *pt > 0.0)
}
pub fn format_font_size(pt: f64) -> String {
    format!("{pt:.3}").trim_end_matches('0').trim_end_matches('.').to_owned()
}
...
mod tests {
    ...
    fn plans_every_desired_state() {
        for applied in [Applied::Unknown, Applied::Base, Applied::Zoomed { delta_pt: 0.625 }, Applied::Zoomed { delta_pt: 1.0 }] {
            for enabled in [false, true] {
                for active in [false, true] {
                    let want = ZoomWant { config: ZoomConfig { enabled, delta_pt: 0.625 }, active };
                    let expected = if enabled && active {
                        if applied == (Applied::Zoomed { delta_pt: 0.625 }) { None } else { Some(ZoomOp::Apply { delta_pt: 0.625 }) }
                    } else if applied == Applied::Base { None } else { Some(ZoomOp::Revert) };
    ...
    fn parses_and_formats() {
        ...
        for bad in ["Alacritty-.sock", "Alacritty-x.sock", "Alacritty-1.log", "foo.sock", "Alacritty-0.sock", "Alacritty-+1.sock"] { assert_eq!(parse_socket_pid(bad), None); }
        assert_eq!(parse_font_size(r#"{"font":{"size":14.0}}"#), Some(14.0));
        for bad in ["{}", r#"{"font":{"size":"14"}}"#, "invalid"] { assert_eq!(parse_font_size(bad), None); }
        for (pt, text) in [(14.625, "14.625"), (15.0, "15"), (14.62500001, "14.625")] { assert_eq!(format_font_size(pt), text); }
    }
}
