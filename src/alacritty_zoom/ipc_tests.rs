...
async fn runner_passes_explicit_arguments_and_strips_inherited_environment() {
    ...
    let instance = dir.create_instance(
        r#"
...
"#,
    ...
    let reply = run_call(&instance, &["get-config".into(), "-w".into(), "-1".into()])
        .await
        .unwrap();
    assert!(reply.success, "{}", reply.stderr);
    assert_eq!(
        crate::alacritty_zoom::parse_font_size(&reply.stdout),
        Some(14.0)
    );
    let reply = run_call(
        &instance,
...
async fn timeout_kills_and_reaps_the_child() {
    ...
    let error = run_call(&instance, &["get-config".into(), "-w".into(), "-1".into()])
        .await
        .unwrap_err();
    assert!(error.to_string().contains("timeout"));
    let pid: i32 = std::fs::read_to_string(dir.0.join("test.sock.pid"))
        .unwrap()
        .parse()
        .unwrap();
    for _ in 0..100 {
...
fn discovery_skips_dead_and_wrong_binary_without_removing_sockets_or_relogging() {
    ...
    let dir = TestDir::create();
    for name in [
        format!("Alacritty-{}.sock", std::process::id()),
        "Alacritty-2147483647.sock".into(),
        "irrelevant.sock".into(),
    ] {
