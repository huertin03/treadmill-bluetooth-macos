...
impl AlacrittyIpc for SystemIpc {
    ...
    fn is_live(&mut self, instance: &AlacrittyInstance) -> bool {
        read_process(instance.identity.pid)
            .is_some_and(|(identity, bin)| identity == instance.identity && bin == instance.bin)
    }
    async fn call(&mut self, instance: &AlacrittyInstance, args: Vec<String>) -> Result<Reply> {
        ensure!(
            self.is_live(instance),
            "Alacritty instance exited or identity changed before IPC"
        );
        run_call(instance, &args).await
    }
    fn zoom_records(&mut self) -> Result<Vec<ZoomRecord>> {
        self.store.zoom_records()
    }
    fn upsert_zoom_record(&mut self, record: &ZoomRecord) -> Result<()> {
        self.store.upsert_zoom_record(record)
    }
    fn delete_zoom_record(&mut self, identity: Identity) -> Result<()> {
        self.store
            .delete_zoom_record(identity.pid, identity.started_at_us)
    }
}

pub fn discover_instances(
    dir: &Path,
    skipped: &mut HashSet<i32>,
) -> Result<Vec<AlacrittyInstance>> {
    let mut instances = Vec::new();
    let mut seen = HashSet::new();
    for entry in std::fs::read_dir(dir)
        .with_context(|| format!("read socket directory {}", dir.display()))?
    {
        let entry = entry.context("read socket directory entry")?;
        let Some(pid) = entry.file_name().to_str().and_then(parse_socket_pid) else {
            continue;
        };
        seen.insert(pid);
        if let Some((identity, bin)) = read_process(pid) {
            skipped.remove(&pid);
            instances.push(AlacrittyInstance {
                identity,
                bin,
                socket: entry.path(),
            });
...
fn read_process(pid: i32) -> Option<(Identity, PathBuf)> {
    ...
    Some((
        Identity {
            pid,
            started_at_us: i64::try_from(started_at_us).ok()?,
        },
        bin,
    ))
}
...
pub async fn run_call(instance: &AlacrittyInstance, args: &[String]) -> Result<Reply> {
        ...
        !args.iter().any(|arg| arg == "--reset"),
        "font-only IPC forbids --reset"
    );
    let output = tokio::time::timeout(IPC_CALL_TIMEOUT, build_command(instance, args).output())
        .await
        .context("Alacritty IPC timeout after 2s")?
        .context("spawn/read Alacritty client")?;
    Ok(Reply {
