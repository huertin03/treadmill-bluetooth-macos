...
#[derive(Clone)]
pub struct AlacrittyZoom { sender: watch::Sender<ZoomWant> }
impl AlacrittyZoom {
    pub fn set_active(&self, active: bool) {
        self.sender.send_if_modified(|want| {
            if want.active == active { return false; }
            want.active = active;
            true
        });
    }
    pub fn set_config(&self, config: ZoomConfig) {
        self.sender.send_if_modified(|want| {
            if want.config == config { return false; }
            want.config = config;
            true
        });
    }
...
pub fn spawn_worker<I: AlacrittyIpc + 'static>(ipc: I, config: ZoomConfig) -> (AlacrittyZoom, JoinHandle<()>) {
    let (sender, receiver) = watch::channel(ZoomWant { config, active: false });
    (AlacrittyZoom { sender }, tokio::spawn(run_worker(ZoomCore::new(ipc), receiver)))
}

async fn run_worker<I: AlacrittyIpc>(mut core: ZoomCore<I>, mut receiver: watch::Receiver<ZoomWant>) {
    ...
    loop {
        ...
        if let Some(op) = plan(applied, &want) {
            if let Err(error) = core.run_op(op).await { tracing::warn!(%error, ?op, "Alacritty zoom reconciliation failed"); }
            applied = match op { ZoomOp::Revert => Applied::Base, ZoomOp::Apply { delta_pt } => Applied::Zoomed { delta_pt } };
            interval.reset();
