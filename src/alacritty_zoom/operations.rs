...
impl<I: AlacrittyIpc> ZoomCore<I> {
    ...
    pub async fn run_op(&mut self, op: ZoomOp) -> Result<()> {
        ...
        if instances.is_empty() {
            tracing::debug!("Alacritty not running");
        }
    ...
    pub async fn rescan(&mut self, delta_pt: f64) -> Result<()> {
        ...
        let instances = self
            ...
            .into_iter()
            .filter(|instance| {
                !self.zoomed.contains(&instance.identity)
                    && !self.failed.contains(&instance.identity)
            })
            .collect();
    ...
    async fn run_instances(&mut self, instances: Vec<AlacrittyInstance>, op: ZoomOp) -> Result<()> {
        ...
        for instance in instances {
            match self.run_instance(&instance, op).await {
                Ok(Some((base, target))) => {
                    if matches!(op, ZoomOp::Apply { .. }) {
                        self.zoomed.insert(instance.identity);
                    }
        ...
        if !outcomes.is_empty() {
            tracing::info!(
                ?op,
                ?outcomes,
                "Alacritty zoom operation completed (pid, base, target)"
            );
        }
        Ok(())
    }
    ...
    async fn run_instance(
        ...
        match op {
            ZoomOp::Apply { delta_pt } => self
                .apply_instance(instance, record, delta_pt)
                .await
                .map(Some),
            ZoomOp::Revert => {
    ...
    async fn apply_instance(
        &mut self,
        instance: &AlacrittyInstance,
        record: Option<ZoomRecord>,
        delta: f64,
    ) -> Result<(f64, f64)> {
        ensure!(
            is_valid_delta(delta),
            "invalid zoom delta: {delta}; expected 0 < pt <= 8"
        );
        ...
        let target = base + delta;
            ...
            target.is_finite(),
            "non-finite Alacritty font target: base={base}, delta={delta}"
        );
        let mut pending = ZoomRecord {
            pid: instance.identity.pid,
            started_at_us: instance.identity.started_at_us,
            socket: instance.socket.to_string_lossy().into_owned(),
            base_pt: base,
            target_pt: target,
            previous_target_pt: owned
                .filter(|_| !sizes_match(current, target))
                .map(|_| current),
    ...
    async fn revert_instance(
        ...
        if changed {
            ...
            tracing::info!(
                pid = instance.identity.pid,
                current,
                recorded_target = record.target_pt,
...
pub fn matches_record(current: f64, record: &ZoomRecord) -> bool {
    sizes_match(current, record.target_pt)
        || record
            .previous_target_pt
            .is_some_and(|pt| sizes_match(current, pt))
}
pub fn record_identity(record: &ZoomRecord) -> Identity {
    Identity {
        pid: record.pid,
