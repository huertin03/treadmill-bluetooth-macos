...
pub(crate) fn print_setting(config: ZoomConfig) {
    let label = if config.enabled { format!("on (+{} pt)", format_font_size(config.delta_pt)) } else { "off".into() };
...
pub(crate) async fn run_zoom(action: Option<ZoomAction>) -> Result<()> {
    match action {
        ...
        Some(ZoomAction::Pt { value }) => {
            if !is_valid_delta(value) { tracing::warn!(value, "invalid Alacritty zoom delta; expected finite 0 < pt <= 8"); }
            ensure!(is_valid_delta(value), "font delta must be finite and 0 < pt <= 8");
        ...
        action => {
            ...
            if matches!(action, Some(ZoomAction::Off)) { set_key("alacritty_zoom", "false")?; }
            ...
            match action {
                ...
                Some(ZoomAction::Preview) => {
                    core.run_op(ZoomOp::Apply { delta_pt: config.delta_pt }).await?;
                    print_probe(&mut core, config).await?;
                    println!("preview: +{} pt; the daemon re-converges on the next presence transition.", highlight_config(format_font_size(config.delta_pt)));
                    ensure!(core.failed.is_empty(), "preview failed for some Alacritty instances; see warnings");
                    Ok(())
                }
                _ => {
                    core.run_op(ZoomOp::Revert).await?;
                    ensure!(core.failed.is_empty(), "font restore failed for some Alacritty instances; records retained for recovery");
...
async fn print_probe(core: &mut ZoomCore<SystemIpc>, config: ZoomConfig) -> Result<()> {
    ...
    if instances.is_empty() { println!("alacritty: not running"); }
    for instance in instances {
        match get_font_size(&mut core.ipc, &instance).await {
            Ok(current) => {
                let record = core.ipc.zoom_records()?.into_iter().find(|record| record_identity(record) == instance.identity && matches_record(current, record));
                let base = record.map_or(current, |record| record.base_pt);
                println!("alacritty: running pid {} — base {} pt → walking {} pt", instance.identity.pid, format_font_size(base), highlight_config(format_font_size(base + config.delta_pt)));
            }
            Err(error) => {
                if !core.ipc.is_live(&instance) {
                    ...
                    println!("alacritty: pid {} exited during probe", instance.identity.pid);
                    continue;
                }
                tracing::warn!(pid = instance.identity.pid, %error, "Alacritty status probe failed");
                println!("alacritty: running pid {} — font unavailable", instance.identity.pid);
            }
        }
    }
...
mod tests {
    ...
    fn parses_all_actions_without_ble_arguments() {
        for arguments in [vec![], vec!["on"], vec!["off"], vec!["pt", "0.625"], vec!["preview"], vec!["reset"]] {
            let cli = crate::Cli::try_parse_from(["tm", "alacritty-zoom"].into_iter().chain(arguments)).unwrap();
            assert!(matches!(cli.command, Some(crate::Commands::AlacrittyZoom { .. })));
        }
